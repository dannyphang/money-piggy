...
export class FormItemComponent implements OnInit, OnDestroy {
  ...
  private renderItem() {
    ...
    if (isBaseButton(this.fieldConfig)) {
      (this.componentRef.instance as ButtonComponent).onClick
  ...
  private invokeDependent() {
    this.fieldConfig.dependOnFields?.forEach((deps) => {
      if (
      ...
      const trackingField = deps.fields.map((field) => {
